...
This proposal starts with a short description of the significant issue that has lead to this project. The sections on  background and on course design give more information on why this signigicant issue has arosen and motivate the specific course design decision for the LBG course. Finally, a solution for the significant issue is proposed.
...
2. The lecturer gets feedback on how well the students are able to apply the course material taught during the lecture of the previous week. This feedback helps the lecturer to identify potential weeknesses in the course material or in its presentation.
...
When looking at the process of knowledge transfer from an abstract point of view, the lecture marks usually the first encounter of the students with the new material. It might not seam so obvious, but also during the lecture student participation and targetted activity can be promoted by using appropriate teaching techniques. Once the students got in contact with the new material, it is important to give them the chance to apply the newly aquired knowledge. One way of such an application is to present them exercises in the sense as they were defined above as a series of tasks where students can apply the previously taught lecture material. This concept of alternating lecture hours and exercise sessions has several benefits. 
...
Prior to an exercise hour the exercise tasks are prepared by the lecturer and are stored in a document. This exercise document together with all the lecture material is copied into each of the student user-spaces on the online platform. Besides the exercise document, students can find in their user space also a copy of the complete lecture material. The lecture material is created using the principles of literate programming that was originally developped by [@Knuth1984]. In literate programming, explanatory text and programming code are intermingled in one common source file. This common source file is compiled which means that the programming code in the source file is executed and the results are collected into a result file. This output file from the compilation process can be converted into different output format. The ideas of literate programming are essential to the paradigm of reproducible research. Research is reproducible when there is enough information available for an independent researcher to repeat the research work and to reproduce the results shown. Reproducibility of research results is a central component in science and constutitues one of the fundamental pillars on which the construction of scientific knowledge is built on. 

When lecturers build their teaching material based on the principles of reproducibility using tools that allow for literate programming, have at least the following to advantages

1. students are introduced into best practices of scientific work using principles of reproducibility
2. principles of reproducibility make the process of producing teaching material very transparent and therebye aiding the students in understanding and in providing more meaningful feedback.

More details on the significance of reproducibility are given in [@Gandrud2014] and [@Xie2015]. 
